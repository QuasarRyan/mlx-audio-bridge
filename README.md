...
```json
{
  "maid": {
    "mode": "voice_design",
    "voice_description": "一位22-25岁左右的私人女仆音色。音质如同沉静的深夜，音调中性偏高，但是完全剥离了尖锐感与甜腻感，呈现出一种厚实且柔和的共鸣感。语气恭敬而克制，说话节奏偏快，带着呼吸音的质感。"
  },
  "storyteller": {
    "mode": "voice_design",
...
这个文件是“扩展 + 覆盖层”，不是必须完整列出所有 OpenAI 原生音色。没写到的 OpenAI voice 会继续使用服务里内置的默认配置。仓库里提供了可选示例文件：[deploy/voices.json](/Users/ryan/works/services/mlx-audio-bridge/deploy/voices.json)。
