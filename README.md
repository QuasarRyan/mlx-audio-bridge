...
| --- | --- | --- |
| `model` | 支持 OpenAI 别名 `gpt-4o-mini-tts`、`tts-1`、`tts-1-hd`，也支持直接传 MLX 模型 ID / 路径，以及 `Qwen3-TTS-12Hz-0.6B-Base`、`Qwen3-TTS-12Hz-0.6B-CustomVoice`、`Qwen3-TTS-12Hz-1.7B-VoiceDesign` 这三个家族名 | OpenAI 别名映射到默认 Base 模型；Qwen3-TTS 家族名会在 `QWEN_MODEL_DIR` 中自动选择兼容量化目录，或回退到对应的 `mlx-community` 默认模型 |
| `input` | 必填文本 | `text` |
