...
| --- | --- | --- |
| `model` | 支持 OpenAI 别名 `gpt-4o-mini-tts`、`tts-1`、`tts-1-hd`，也支持直接传 MLX 模型 ID / 路径，以及 `Qwen3-TTS-12Hz-0.6B-Base`、`Qwen3-TTS-12Hz-1.7B-Base`、`Qwen3-TTS-12Hz-0.6B-CustomVoice`、`Qwen3-TTS-12Hz-1.7B-VoiceDesign`、`Qwen3-TTS-12Hz-1.7B-CustomVoice` 这五个家族名 | OpenAI 别名会按音色模式自动选家族：`voice_design -> 1.7B VoiceDesign`，`custom_voice -> 0.6B CustomVoice`，`voice_clone -> Base（优先 1.7B，其次 0.6B）`；显式传家族名时会在 `QWEN_MODEL_DIR` 中自动选择兼容量化目录，或回退到对应的 `mlx-community` 默认模型 |
| `input` | 必填文本 | `text` |
...
| `QWEN_TTS_MODEL_NAME` | 空 | 可选的 TTS 模型子目录名；未设置时会先在 `QWEN_MODEL_DIR` 中按 `8bit`、`6bit`、`5bit`、`4bit`、`bf16` 的顺序自动选择兼容的 `Qwen3-TTS-12Hz Base 1.7B` 本地目录；如果本地没有 `1.7B Base`，则继续尝试本地 `0.6B Base`；两者都没有时回退到 Hugging Face 上的 `1.7B Base` |
