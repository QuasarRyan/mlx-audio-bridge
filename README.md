...
| `repetition_penalty` | 可选；默认 `1.05`。如果是 Base 模型的 ICL 语音克隆路径（`voice_clone`），小于 `1.5` 的值会自动提升到 `1.5` | `repetition_penalty` |
...
当前服务还会固定传入一组偏稳定的 Qwen3-TTS 采样参数：`temperature=0.6`、`top_p=0.9`、`top_k=30`。`repetition_penalty` 也已接入；默认是 `1.05`。如果走 Base 模型的 ICL 语音克隆路径（`voice_clone`），小于 `1.5` 的值会被提升到 `1.5`，因此在这个场景下设置小于 `1.5` 没有实际意义。
